...
@Component({
  ...
  styleUrl: './search-4.component.scss',
  imports: [
    FormsModule,
    ReactiveFormsModule,
    NgClass,
...
export class Search4Component implements AfterViewInit, OnDestroy {
  ...
  handleBlur() {
    // Disable animation for static placeholder
    // this.searchAnim();
    // this.char = 0;
  }
  ...
  /**
   * Check if header background is white
   */
  isHeaderWhite(): boolean {
    const bgColor = this.shopInfo?.headerBgColor?.toLowerCase()?.trim();
    return bgColor === '#fff' || bgColor === '#ffffff' || bgColor === 'white';
  }
