...
export class ProductCard1Component implements OnInit, OnDestroy, OnChanges {
  ...
  onAddToCart(event: MouseEvent, type: 'addToCart' | 'buyNow') {
    ...
    if (shouldUseDirectCheckout(this.shopId) && type === 'buyNow') {
  ...
  private setDefaultVariation() {
    ...
    // If resolved combo is OOS, clear it so pricing pipes don’t bind to a bad variant
    if (
      this.selectedVariationList &&
  ...
  onSelectVariation2(name: string) {
    ...
    if (this.selectedVariation2 !== name) {
      ...
      this.setSelectedVariationList();
      // Reset qty if exceeds new stock for the new selection
      if (this.qty > this.maxQty) this.qty = Math.max(1, this.maxQty);
    }
  }
  ...
  /** Max qty allowed based on stock and selection */
  get maxQty(): number {
    return 9999;
  }
