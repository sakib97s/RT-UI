...
// Custom Order Modal
  ...
  // Variation Manage
  selectedVariation: string = null; // value for product.variation (e.g., Color)
  selectedVariation2: string = null; // value for product.variation2 (e.g., Size)
  ...
  ngOnInit() {
    ...
    // Set initial quantity based on cart if product is already in cart
    if (this.cart) {
  ...
  onAddToCart(event: MouseEvent, type: 'addToCart' | 'buyNow') {
    ...
    // Open modal first for variation products
  ...
  // ===== Variation Control =====
  /** Build internal normalized variant list (v1/v2) from product.variationList */
  ...
  /** Find the first in-stock combination (if any) */
  ...
  /** Choose defaults prioritizing an in-stock combination */
  private setDefaultVariation() {
    if (!this.product?.isVariation) return;

    // Prefer first IN-STOCK combo
    ...
    if (firstOk) {
      ...
      // Nothing in stock → fall back to first options (won’t be selectable anyway)
    ...
    // If resolved combo is OOS, clear it so pricing won’t bind to a bad variant
    if (
      this.selectedVariationList &&
  ...
  /** Disable a first-dimension option (variation) when all combos for it are OOS */
  isV1Disabled(v1: string): boolean {
    return false;
  }

  /** Disable a second-dimension option (variation2) when all combos for it are OOS */
  ...
  onSelectVariation(name: string) {
    ...
    if (this.selectedVariation !== name) {
      ...
      // Color image swap (keep your logic)
      if (
