...
export class ProductCard4Component implements OnInit, OnDestroy, OnChanges {
  // Decorator
...
// Custom Order Modal
  ...
  // Variation Manage
  selectedVariation: string = null; // product.variation value (e.g., Color)
  selectedVariation2: string = null; // product.variation2 value (e.g., Size)
  ...
  checkCartList() {
    this.cart = this.carts.find((f) => {
      const productId =
        typeof f.product === 'string' ? f.product : (f.product as Product)?._id;
      const isProductMatch = productId === this.product?._id;

      const isVariationMatch =
        (!f.variation && !this.selectedVariationList) ||
        (f.variation &&
          this.selectedVariationList &&
          f.variation._id === this.selectedVariationList._id);
  ...
  // ===== Variation Control =====
  /** Normalize product.variationList to an easier structure */
  ...
  /** Choose defaults prioritizing an in-stock combo */
  private setDefaultVariation() {
    ...
    if (firstOk) {
      ...
      // Nothing in stock → just take first options
    ...
    // If resolved combination is OOS, clear it so price pipes don't bind wrongly
    if (
      this.selectedVariationList &&
  ...
  onSelectVariation2(name: string) {
    ...
    if (this.selectedVariation2 !== name) {
      ...
      this.setSelectedVariationList();
      // Clamp qty to available stock for new selection
      if (this.qty > this.maxQty) this.qty = Math.max(1, this.maxQty);
    }
  }
