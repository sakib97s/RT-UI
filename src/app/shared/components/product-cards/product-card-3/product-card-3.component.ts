...
export class ProductCard3Component implements OnInit, OnDestroy, OnChanges {
  // Decorator
  ...
  private setDefaultVariation() {
    ...
    if (firstOk) {
      ...
      // Nothing in stock → fallback to first options (they'll be disabled anyway)
    ...
    // If resolved combo is OOS, clear it so pricing doesn’t bind to a bad variant
    if (
      this.selectedVariationList &&
